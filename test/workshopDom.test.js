...
function compound(chunk) {
  const pseudoEl = (chunk.match(/::[\w-]+/) ?? [null])[0];
  const bare = chunk.replace(/::[\w-]+/g, '');
  return {
    pseudoEl,
    classes: (bare.match(/\.[\w-]+/g) ?? []).map((c) => c.slice(1)),
    tag: /^[a-z][\w-]*/i.exec(bare)?.[0] ?? null,
  };
}
 ...
 */
function applies(ruleSelector, targetSelector) {
  const split = (s) => s.trim().split(/\s*[>+~]\s*|\s+/);
  const rule = split(ruleSelector);
  const target = split(targetSelector);
  const r = compound(rule[rule.length - 1]);
  const t = compound(target[target.length - 1]);
  ...
  if (!r.classes.every((c) => t.classes.includes(c))) {
    return false;
  }
  return rule.slice(0, -1).every((ancestor) => target.slice(0, -1).includes(ancestor));
}
 ...
 */
function mediaActive(media, reduce) {
  return media.every((m) => (/prefers-reduced-motion/.test(m) ? /reduce/.test(m) === reduce : true));
}

/** Hodnota vlastnosti, která na daném prvku vyhraje (null = nikdo ji nenastavuje). */
function resolveValue(rules, targetSelector, prop, reduce = false) {
  const winner = rules
...
function resolveAnimation(rules, selector, reduce) {
